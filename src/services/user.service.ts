...
export class SuggestionService {
  static async getSuggestions(userEmail: string): Promise<SuggestInfoDto[]> {
    ...
    // 1순위: 완벽히 일치하는 사용자
    const firstPriority =
      await SuggestionRepository.findMatchingUsersCertifications({
        age,
        department,
        categoryNames,
      });
    ...
    const secondPriority =
      await SuggestionRepository.findPartiallyMatchingUsersCertifications({
        age,
        department,
        categoryNames,
      });
    ...
    const defaultRecommendations =
      await SuggestionRepository.findDefaultCertificationsByCategory(
        categoryNames
      );
...
export const getClosestExams = async (userId: number): Promise<any[]> => {
  ...
  return exams.map((exam) => ({
    ...
    dDay: Math.ceil(
      ...
      // (exam.examDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24)
    ),
  }));
};
